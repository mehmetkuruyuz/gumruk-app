...
<div class="col-md-4">
	@if (!empty($operasyonList))
		<select class="form-control" name="x" id="xx" onchange="openfile()">

				<option value="0">{{trans("messages.choose")}}</option>
		@foreach($operasyonList as $m=>$evm)
				<option value="{{$m+1}}">{{$evm->name}}</option>
		@endforeach
		</select>
	@endif
</div>
<hr />
		<div class='col-md-12'>
						...
						@if (!empty($operasyonList))
							@foreach($operasyonList as $m=>$evm)
								<div class="card d-none allfiles" id="firma{{$m+1}}">
