													 ...
													 @if (!empty($evm->evrak[0]))
														 ...
														 <a href='javascript:void(0)' class="btn btn-info btn-sm" onclick="seciliindir(this)">{{trans("messages.seciliindir")}}</a><br  />
															@foreach ($evm->evrak as $evkey => $evvalue)
																	<a href='/uploads/{{$evvalue->fileName}}' class="btn btn-alert btn-sm" target="_blank">{{($evvalue->kacinci)+1}}. {{trans("messages.gumruk")}} {{-- trans("messages.yuk") --}} {{$evvalue->belgetipi}}- {{($evvalue->siraId)+1}} {{trans("messages.indir")}}</a>
