...
function noktalarIcinAlanOlustur(t,kacinci)
{
	...
	for (i = 1; i <= adet; i++)
	{
		...
		'<tr>'+
            ...
            '<select name="talimatTipi['+kacinci+'][]" class="form-control" id="talimatTipi" onchange="karneBilgisiEkle(this)">'+
            ...
            '<td><input type="text" class="form-control  hesaplanacakKilo" name="tekKilo['+kacinci+'][]" required="required"  onchange="hesaplaKapKilo('+kacinci+')"/></td>'+
						...
						'<td><input type="text" class="form-control" name="faturabedeli['+kacinci+'][]" required="required"   /> </td>'+
...
function addTabData()
{
	   ...
	   function(data, status){
		...
		for(t=1;t<=say;t++)
		{
				$("#myTab").append('<li class="nav-item"><a class="nav-link active " id="linktab-'+t+'" data-toggle="tab" href="#tab-'+t+'" role="tab" aria-controls="tab-'+t+'" aria-selected="false"> '+(t)+'</a>');
		}
...
<form action='/operasyon_new/save' method="post" name='a' id='actionFormElement' enctype="multipart/form-data"   onsubmit="return confirm('{{trans("messages.formconfirm")}}');">
{{ csrf_field() }}
      <div class='row' style='margin-left:2px;margin-top:4px;background: #FFF;'>
            ...
            <div id='idXLKO'></div>
              <ul class="nav nav-tabs col-md-12" id="myTab" role="tablist">
               <li class="nav-item"><a class="nav-link active " id="bir-tab" data-toggle="tab" href="#bir" role="tab" aria-controls="bir" aria-selected="false"> 1</a>
              </ul>
                			...
                			<table class="table table-bordered malYuklemeBosaltmaTablolari" width="100%" cellspacing="0">
                              ...
                              <td><input type="text" class="form-control hesaplanacakKap" name="tekKap[0][]" required="required" onchange="hesaplaKapKilo(0)"/></td>
            							    <td><input type="text" class="form-control  hesaplanacakKilo" name="tekKilo[0][]" required="required"  onchange="hesaplaKapKilo(0)"/></td>
															...
															<td><input type="text" class="form-control" name="faturabedeli[0][]" required="required"   /> </td>
															<td>
