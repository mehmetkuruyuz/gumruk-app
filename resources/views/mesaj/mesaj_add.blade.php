            ...
            <!-- /.box-header -->
                <div class="box-body">
                  <div class="form-group">
                        <label for="messageUserTO">{{trans('messages.sendinguser')}}</label>
                        <select class="form-control" id="messageUserTO" name='userTo' required="required">
                          <option value="0">{{trans('messages.choose')}}</option>
                          @if (!empty($users))
                          	@foreach($users as $key=>$val)
                          	<option value="{{$val['id']}}"  @if ((!empty($ozelId)) && ($ozelId==$val['id'])) selected="selected" @endif >{{$val['name']}} ({{$val['email']}})</option>
                          	@endforeach
                          @endif
                        </select>
                  </div>
