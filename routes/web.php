...
Route::group(['middleware' =>[ 'web']], function ()
{
    ...
    Route::get("/yetkilendirme","AdminController@yetki")->middleware("auth");
    Route::post("/yetkilendirme/save","AdminController@yetkisave")->middleware("auth");
    Route::post("/yetkilendirme/show","AdminController@searchallauth")->middleware("auth");


    Route::get("/testauth","TestOperationController@doneindex");

    Route::get('/','HomeController@homePage')->name('home')->middleware('auth');;
    /* Mesaj BÖLÜMÜ */
    Route::get('/mesaj', 'MesajController@index')->name('MesajController-index')->middleware('auth');
    Route::get('/mesaj/sent', 'MesajController@sent')->name('MesajController-sent')->middleware('auth');
    Route::get('/mesaj/deleted', 'MesajController@deleted')->name('MesajController-deleted')->middleware('auth');
    Route::get('/mesaj/yeni/{userId?}', 'MesajController@add')->name('MesajController-yeni')->middleware('auth');
    Route::post('/mesaj/save', 'MesajController@save')->name('MesajController-save')->middleware('auth');
    Route::get('/mesaj/sil/{id}','MesajController@delete')->name('MesajController-delete')->middleware('auth');
    Route::get('/mesaj/read/{id}','MesajController@read')->name('MesajController-read')->middleware('auth');
    Route::get('/mesaj/cevapla/{id}','MesajController@cevapla')->name('MesajController-cevapla')->middleware('auth');



    Route::get('/arac/ihracat/new','TalimatController@new')->name("TalimatController-arsiv-new")->middleware("auth");
    Route::get('/arac/ithalat/new','TalimatController@ithalatnew')->name("TalimatController-arsiv-ithalat-new")->middleware("auth");
    Route::get('/arac/talimatgetir/{talimat}/{say}/{firmaId?}','TalimatController@altdata')->name("TalimatController-arsiv-ihracat-altdata")->middleware("auth");
    Route::post("/arac/ihracat/save","TalimatController@ihracatsave")->name("TalimatController-arsiv-ihracat-savedata")->middleware("auth");
    Route::post("/arac/ithalat/save","TalimatController@ithalatsave")->name("TalimatController-arsiv-ihracat-savedata")->middleware("auth");
    Route::post("/arac/ihracat/savet2","TalimatController@ihracatsavet2")->name("TalimatController-arsiv-ihracat-savedata")->middleware("auth");


    Route::any('/operation/continue', 'OperationController@index')->name('OperationController.yeni.new')->middleware('auth');
    //Route::any('/operation/done', 'OperationController@doneindex')->name('operasyon.yeni.new')->middleware('auth');
    Route::any('/operation/done', 'OperationController@newdoneindex')->name('OperationController.donenew')->middleware('auth','role:check');
    Route::any('/operationGetToBack/{id}', 'OperationController@getCompanyDoneList')->name('OperationController.done.list.compnay')->middleware('auth');


    ##Yeni İhracat Kayıtları bu alanda
    Route::get("/ihracat/arac/new","IhracatController@new")->name("IhracatController-ihracat-yeni-new")->middleware("auth");
    Route::get("/ihracat/gumrukgetir/{say}","IhracatController@gumrukdatagetir")->name("IhracatController-gumruk-yeni-new")->middleware("auth");
    Route::get("/ihracat/bilgigetir/{tip}/{say}","IhracatController@talimattipigetir")->name("IhracatController-gumruk-yeni-new")->middleware("auth");
    Route::post("/ihracat/arac/save","IhracatController@ihracatsave")->name("IhracatController-ihracat-savedata")->middleware("auth");
    Route::get("/ihracat/operasyon/list","IhracatController@ihracatlist")->name("IhracatController-ihracat-ihracatlist")->middleware("auth");
    ...
    Route::get("/ihracat/operasyon/edit/{id}","IhracatController@ihracatedit")->name("gumruk-yeni-new")->middleware("auth");
    ...
    Route::get('/ihracat/operasyon/altparametre/delete/{id}','IhracatController@ihracataltdelete')->name('operasyon.yeni.done')->middleware('auth');
    Route::post("/ihracat/operasyon/update",'IhracatController@ihracatUpdate')->name('operasyon.yeni.upload')->middleware('auth');

    Route::get('/ihracat/operasyon/sub/upload', 'IhracatController@evrakscreensub')->name('operasyon.yeni.upload')->middleware('auth');
    Route::post("/ihracat/operasyon/file_sub/upload",'IhracatController@evrakuploadsub')->name('operasyon.yeni.upload')->middleware('auth');
    Route::get("/ihracat/dosya/indirfull/{specialid}",'IhracatController@topludisariindir')->name('operasyon-save');




    Route::any('/ihracat/muhasebe/kapalifatura/', 'IhracatMuhasebeController@kapalifatura')->name('muhasebe-kaydet-ozel')->middleware('auth');
    Route::any('/ihracat/muhasebe/acikfatura/', 'IhracatMuhasebeController@acikfatura')->name('muhasebe-kaydet-ozel')->middleware('auth');
    Route::get('/ihracat/muhasebe/fatura/{id}', 'IhracatMuhasebeController@faturakapat')->name('muhasebe-kaydet-ozel')->middleware('auth');
    Route::any('/ihracat/muhasebe', 'IhracatMuhasebeController@index')->name('muhasebe')->middleware('auth');
    Route::get('/ihracat/muhasebe/faturayazdir/{id}', 'IhracatMuhasebeController@print')->name('muhasebe-kayit')->middleware('auth');
    Route::get('/ihracat/muhasebe/kapa/{id}', 'IhracatMuhasebeController@faturaode')->name('muhasebe-kaydet-ozel')->middleware('auth');
    Route::get('/ihracat/muhasebe/makbuz/{id}', 'IhracatMuhasebeController@makbuzyazdir')->name('muhasebe-makbuz-ozel')->middleware('auth');

    Route::any('/ihracat/muhasebe/raporlama', 'IhracatMuhasebeController@raporlama')->name('muhasebe-a-ozel')->middleware('auth');

    Route::any('/ihracat/gunsonuraporu', 'IhracatMuhasebeController@gunsonuraporu')->name('yuklistesi')->middleware('auth');
    ...
    Route::get("/ihracatext/done/{tarih?}/{tarih2?}", 'IhracatController@exceloperation')->name('ihracat.yeni.a')->middleware('auth');;
    ...
    Route::get('/talimat/yukcinsi', 'TalimatController@yukcinsiListesi')->name('yuklistesi')->middleware('auth')->middleware('auth');;

    Route::get('/users/emaillist/{id}', 'Users@emaillist')->name('emaillistesi')->middleware('auth')->middleware('auth');;




/* YENI MUHASABE KALIBI BURADAN DEVAM EDECEK */

      Route::get('/muhasebe/new/ozelfiyatolusturma', 'MuhasebeEkController@ozelfiyatolusturma')->name('ozelfiyatolusturma')->middleware('auth');
      Route::get('/muhasebe/yeni/ozelparametreler/{id}', 'MuhasebeEkController@ozelfiyataltparametre')->name('muhasebe-ozelparametreler-yeni')->middleware('auth');
      Route::post("/muhasebe/yeni/ozelfiyatlamakaydet",'MuhasebeEkController@ozelfiyatsave')->name("muhasebe-ozelparametreler-save")->middleware('auth');


      Route::get("/unauthorizedarea",function(){return view("layouts.yetki-alani");});
...
*/
     ...
     Route::get("/dosya/indirfull/{specialid}",'IhracatController@topludisariindir')->name('operasyon-save');
