...
class MesajController extends Controller
{
    ...
    public function  index()
    {
        ...
        return view('mesaj.mesaj_index',['mesajlar'=>$mesajlar,'type'=>"inbox"]);
    ...
    public function sent()
    {
        ...
        return view('mesaj.mesaj_index',['mesajlar'=>$mesajlar,'type'=>"send"]);
    ...
    public function deleted()
    {
        ...
        return view('mesaj.mesaj_index',['mesajlar'=>$mesajlar,'type'=>"deleted"]);
    }
    ...
    public function save(Request $request)
    {
        ...
        \Mail::to('interbos@bosphoregroup.com')->cc($userEmail)->send(new InfoMailEvery($userFirmName,$userEmail,trans('messages.newmessage'), $mesajModel->mesajIcerigi)) ;
    ...
    public function read($_id)
    {
        ...
        if ($userRole=="admin")
        {
            $mesajlar=$mesajModel->with(['ToUser','FromUser'])->where("id",'=',$_id)->orderBy('id','desc')->get();
            $mesajlar=json_decode($mesajlar,True);

        }else
        {
            $mesajlar=$mesajModel->with(['ToUser','FromUser'])->where("id",'=',$_id)->where("toUser" ,"=", $userId)->orWhere('fromUser' ,'=', $userId)->get();
            $mesajlar=json_decode($mesajlar,True);
            //   return $mesajlar;
        }
